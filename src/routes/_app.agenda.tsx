      ...
      <div className="grid gap-4 lg:grid-cols-3">
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {Array.from({ length: diasMes }).map((_, i) => {
                ...
                return (
                  <div
                    key={dia}
                    className={`min-h-[64px] rounded-lg border border-[var(--card-border)] p-1.5 text-left text-xs transition-colors hover:bg-[var(--card-border)]/30 ${
                      isHoje ? 'bg-brand-500/10 ring-1 ring-brand-500/40' : ''
                    }`}
                  >
                    <span className="font-semibold">{dia}</span>
                    {evs.slice(0, 2).map((e) => (
                      <p key={e.id} className="mt-0.5 truncate rounded bg-brand-500/15 px-1 text-[10px] text-brand-600 dark:text-brand-300">
                        {e.titulo}
                      </p>
                    ))}
                    {evs.length > 2 && (
                      <p className="text-[10px] text-muted">+{evs.length - 2}</p>
                    )}
                  </div>
                )
              })}
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {proximos.map((e) => (
              <div key={e.id} className="flex gap-3 rounded-lg border border-[var(--card-border)] p-3">
                <div className="flex size-11 shrink-0 flex-col items-center justify-center rounded-lg bg-brand-500/15 text-brand-600 dark:text-brand-300">
