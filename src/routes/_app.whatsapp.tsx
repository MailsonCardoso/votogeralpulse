      ...
      <div className="mt-4 grid min-h-0 flex-1 gap-4 lg:grid-cols-[300px_1fr_280px]">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            {conversas.map((c) => (
              <button
                ...
                className={cn(
                  'flex w-full items-center gap-3 border-b border-[var(--card-border)]/60 p-3 text-left transition-colors',
                  ativa.id === c.id ? 'bg-brand-500/10' : 'hover:bg-[var(--card-border)]/30',
                )}
              >
                <div className="relative">
                  <AvatarInitials name={c.nome} className="size-9" />
                  {c.online && (
                    <span className="absolute -bottom-0.5 -right-0.5 size-3 rounded-full bg-[var(--color-success)] ring-2 ring-[var(--card)]" />
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium">{c.nome}</p>
                  <p className="truncate text-xs text-muted">{c.bairro}</p>
                </div>
                {c.naoLidas > 0 && (
        ...
        <Card className="flex min-h-0 flex-col overflow-hidden">
          <div className="flex items-center gap-3 border-b border-[var(--card-border)] p-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted">{ativa.online ? 'online' : 'offline'}</p>
            </div>
          </div>
          <div className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-[var(--bg-elevated)] p-4">
            {mensagens.map((m) => (
              <div key={m.id} className={cn('flex', m.de === 'cabo' ? 'justify-end' : 'justify-start')}>
                ...
                >
                  <p>{m.texto}</p>
                  <p className={cn('mt-0.5 text-[10px]', m.de === 'cabo' ? 'text-white/70' : 'text-muted')}>
                    {m.hora}
                  </p>
                </div>
              </div>
        ...
        <Card className="flex min-h-0 flex-col overflow-hidden p-4">
          <div className="border-b border-[var(--card-border)] pb-3">
            ...
            <p className="mt-2 text-xs text-muted">{ativa.bairro}</p>
          </div>
          <p className="py-3 text-xs font-semibold uppercase tracking-wider text-muted">
            Templates
          </p>
          <div className="min-h-0 flex-1 space-y-2 overflow-y-auto">
            {templates.map((t) => (
              <button
                ...
                className="block w-full rounded-lg border border-[var(--card-border)] p-3 text-left text-xs transition-colors hover:bg-[var(--card-border)]/30"
              >
                <p className="mb-1 font-medium text-brand-600 dark:text-brand-300">{t.nome}</p>
                <p className="line-clamp-2 text-muted">{t.texto}</p>
              </button>
            ))}
          </div>
        </Card>
